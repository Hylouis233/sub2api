...
func calculateOpenAI429ResetTime(headers http.Header) *time.Time {
	...
	// Neither Codex usage window is exhausted. The x-codex reset headers
	// describe quota-window resets, not the transient burst/RPM cooldown that
	// produced this 429. Returning nil lets callers use the short fallback
	// cooldown instead of freezing the account until the longest quota window.
