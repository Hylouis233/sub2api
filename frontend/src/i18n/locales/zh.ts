  ...
  admin: {
    ...
    accounts: {
      ...
      bulkActions: {
        ...
        refreshTokenSuccess: '已成功重登 {count} 个账号',
      ...
      oauth: {
        ...
        openai: {
          ...
          accessTokenDesc: '输入 OpenAI accessToken，支持批量输入（每行一个）。未带 refresh_token 时会按 accessToken 过期时间管理。',
          accessTokenInputLabel: 'Access Token',
          accessTokenPlaceholder: '粘贴 OpenAI accessToken...\n支持多个，每行一个',
          accessTokenHint: '适合临时导入 AT；如需自动续期，请优先使用 RT 或 Codex JSON。',
          accessTokenImportAndSave: '导入并保存',
          singleReauthInputRequired: '现有账号重登一次只能输入一个 {label}',
          ...
          codexSessionImportAndCreate: '导入并保存',
    ...
    proxies: {
      ...
      subscriptionSupportedHint: '当前仅会导入 http、https、socks5、socks5h 节点；SS/VLESS/Hysteria/Trojan 等节点会被跳过。',
      subscriptionImportButton: '导入订阅',
      subscriptionImportRequired: '请填写订阅 URL 或粘贴订阅内容',
      subscriptionImportSuccess: '订阅导入完成：创建 {created}，跳过 {skipped}，不支持 {unsupported}',
      subscriptionImportPartial: '订阅导入完成但有问题：创建 {created}，跳过 {skipped}，不支持 {unsupported}，无效 {invalid}，失败 {failed}',
